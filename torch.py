...
class Accumulator:  
    """在n个变量上累加"""
    def __init__(self, n):
        self.data = [0.0] * n   # 该操作可以创造一个长度为n的列表 

    def add(self, *args):
        self.data = [a + float(b) for a, b in zip(self.data, args)]   # zip() 用于将可迭代的对象（如列表、元组、字典等）合并为一个元组的列表，其中的元组包含了来自每个输入可迭代对象的元素。
                                                                      # 函数会按照最短的输入序列截断，也就是说，如果输入的序列长度不一，那么生成的元组列表的长度将等于最短的那个序列的长度。
    def reset(self):
        self.data = [0.0] * len(self.data)
